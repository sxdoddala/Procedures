create or replace PROCEDURE      ttec_h_br_cnv_sal AS
 -- ---------------------------------------------------------------------
 -- ---------------------------------------------------------------------
 ...
 -- ---------------------------------------------------------------------
     ...
     -- --------------------------------------------------------------------------
      ...
      -- -------------------------------------------------------------------------
               ...
               e_log_file_name                         VARCHAR2(20) :='HR_SAL_BR_log_file_';
...
FROM
			   ...
			   /*cust.ttec_h_br_emp emp_t				-- Commented code by IXPRAVEEN-ARGANO,19-May-2023
               ,cust.ttec_h_br_asg asg_t
               ,cust.ttec_h_br_sal sal_t
               ,hr.per_all_people_f papf
               ,hr.per_all_assignments_f paaf*/
			    apps.ttec_h_br_emp emp_t				--  code Added by IXPRAVEEN-ARGANO,   19-May-2023
               ,apps.ttec_h_br_asg asg_t
               ,apps.ttec_h_br_sal sal_t
     ...
     -- --------------------------------------------------------------------------------------------------
        ...
        /*       -- ------------------------------------------------------------
               BEGIN
                       ...
                       INTO l_emp_count
                       FROM cust.ttec_h_br_emp emp_t, cust.ttec_h_br_sal sal_t
...
LOOP
 ...
 ' ********'||SYSTEM_USER_CONST);
	...
	--	  v_inv_next_sal_date_warning   := null ;
  	...
  	-- -----------------------------------------------------------------------------------------
        --      Check for business group id  -- for BR salary source data, business group name is provided
        --                                          and column name in cust.ttec_h_br_sal table is business_group_name
        -- -----------------------------------------------------------------------------------------
-- xxx chaned to check it against per_business_groups and BR gets name  from source data in business_group_info column
...
apps.ttec_conv_util.Stampln(e_open_log_file, 'DISPLAY 1 ' ||
'v_pay_proposal_id=' || v_pay_proposal_id ||
'xSal.assignment_id=' || xSal.assignment_id ||
'l_business_group_id=' || l_business_group_id
);

apps.ttec_conv_util.Stampln(e_open_log_file, 'DISPLAY  2' ||'xsal.change date='||xsal.change_date||
'xsal.effective start date=' || xsal.effective_start_date ||
'xSal.proposal_reason=' || xSal.proposal_reason || ' l_proposed_salary_n=' || l_proposed_salary_n
               ...
               IF (NOT l_error  AND NOT l_sal_err_flag) THEN
				...
				-- UPDATE cust.ttec_h_br_sal			-- Commented code by IXPRAVEEN-ARGANO,19-May-2023
				 UPDATE apps.ttec_h_br_sal              --  code Added by IXPRAVEEN-ARGANO,   19-May-2023
                                 SET
				...
				EXCEPTION
				      ...
				      WHEN OTHERS THEN
				       ...
				       l_sal_err_flag := TRUE ;
                                       ...
                                       g_error_message := SQLERRM;
                                       l_error_message :=  'ERROR while update ttec_h_br_sal temporary talbe ';
                                               apps.ttec_conv_util.Stampln(e_open_log_file, MODULE_NAME_CONST||' '|| l_error_message
