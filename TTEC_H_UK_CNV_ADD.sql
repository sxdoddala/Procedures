create or replace PROCEDURE TTEC_H_UK_CNV_ADD AS
  -- -----------------------------------------------------------------------------
  -- -----------------------------------------------------------------------------
  -- Program Name  : TTEC_H_UK_CNV_ADD
  -- Author        : Pradip Kumar Hati
  -- Creation Date : 02/24/05
  -- Purpose       : Program to load TTEC UK Employee Address data from Legacy System
  -- -----------------------------------------------------------------------------
  -- -----------------------------------------------------------------------------
 ...
 e_log_file_name                VARCHAR2(20)   := 'HR_ADD_UK_log_file_';
...
FROM   cust.ttec_h_uk_add thua
      ,cust.ttec_h_uk_emp thue
*/
--code added by RXNETHI-ARGANO
FROM   apps.ttec_h_uk_add thua
      ,apps.ttec_h_uk_emp thue
--END R12.2 Upgrade Remediation
WHERE  thue.employee_number_in_ls =   thua.employee_number_in_ls
AND    thue.status = 'S'
AND    nvl(thua.status,'X') != 'S' ;
...
--  ---------------------------------------------------------------------
BEGIN
	IF ltrim(rtrim(r_add.country))='GB'THEN
...
--  ---------------------------------------------------------
  ...
  (p_validate                     => FALSE
  ,p_effective_date               => r_add.effective_date
  ...
  ,p_address_line1                => r_add.address_line1
  ,p_address_line2                => r_add.address_line2
  ,p_address_line3                => r_add.address_line3
  ,p_town                         => r_add.town_or_city
  ,p_county                       => r_add.region_1
  ,p_postcode                     => r_add.postal_code
  ,p_country                      => r_add.country
  ,p_telephone_number             => r_add.telephone_number
  ...
  ,p_add_information13            => r_add.add_information13
  ,p_add_information14            => r_add.add_information14
  ,p_add_information15            => r_add.add_information15
  ,p_add_information16            => r_add.add_information16
...
-- --------------------------------------------------------------
   ...
   IF (o_address_id is not null) THEN
		--UPDATE cust.ttec_h_UK_add    --code commented by RXNETHI-ARGANO,19/05/23
		UPDATE apps.ttec_h_UK_add      --code added by RXNETHI-ARGANO,19/05/23
		SET
			address_id	= o_address_id,
			party_id 	= o_party_id,
                        person_id       = r_add.person_id,
			STATUS   	= STATS
