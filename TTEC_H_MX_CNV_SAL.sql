create or replace PROCEDURE      ttec_h_mx_cnv_sal AS
 -- ---------------------------------------------------------------------
 -- ---------------------------------------------------------------------
 -- Author: IT-Convergence - ken Park
 ...
 -- ---------------------------------------------------------------------
     ...
     -- --------------------------------------------------------------------------
      ...
      -- -------------------------------------------------------------------------
               ...
               e_log_file_name                         VARCHAR2(20) :='HR_SAL_MX_log_file_';
  ...
  CURSOR cSal is
     SELECT
               ...
               ,emp_t.hire_date
               ,emp_t.assignment_id
               ,sal_t.employee_number_in_ls
               ,sal_t.business_group_name
...
FROM
		--START R12.2 Upgrade Remediation
               /*cust.ttec_h_mx_emp emp_t				-- Commented code by IXPRAVEEN-ARGANO,19-May-2023
               ,cust.ttec_h_mx_asg asg_t
               ,cust.ttec_h_mx_sal sal_t
               ,hr.per_all_people_f papf
               ,hr.per_all_assignments_f paaf*/
			    apps.ttec_h_mx_emp emp_t				--  code Added by IXPRAVEEN-ARGANO,   19-May-2023
               ,apps.ttec_h_mx_asg asg_t
               ,apps.ttec_h_mx_sal sal_t
               ,apps.per_all_people_f papf
               ,apps.per_all_assignments_f paaf
		--END R12.2.12 Upgrade remediation	   
WHERE
               emp_t.employee_number_in_ls = sal_t.employee_number_in_ls
 AND           emp_t.employee_number_in_ls = asg_t.employee_number_in_ls
 AND           emp_t.status ='S'
 AND           asg_t.status ='S'
 AND           (sal_t.status is null OR sal_t.status = 'F')
     ...
     -- --------------------------------------------------------------------------------------------------
        ...
        /*       -- ------------------------------------------------------------
               BEGIN
                       ...
                       INTO l_emp_count
                       FROM cust.ttec_h_mx_emp emp_t, cust.ttec_h_mx_sal sal_t
                       WHERE emp_t.status = 'S'
                       AND emp_t.employee_number_in_ls = sal_t.employee_number_in_ls;
...
LOOP
 ...
 || xSal.employee_number_in_ls || 'person_id=' || xSal.person_id ||
 ' ********'||SYSTEM_USER_CONST);
  	...
  	-- -----------------------------------------------------------------------------------------
        --      Check for business group id  -- for MX salary source data, business group name is provided
        --                                          and column name in cust.ttec_h_mx_sal table is business_group_name
        -- -----------------------------------------------------------------------------------------
-- xxx chaned to check it against per_business_groups and MX gets name  from source data in business_group_info column
IF (xSal.business_group_name is NOT null) THEN
BEGIN
               SELECT  business_group_id
                   INTO  l_business_group_id
                   FROM  per_business_groups a
                  WHERE a.name = xSal.business_group_name
...
'pay_basis_id=' || xSal.pay_basis_id
);
-- ken1
...
'xSal.proposal_reason=' || xSal.proposal_reason || ' l_proposed_salary_n=' || l_proposed_salary_n
);

/* -- here need to validate pay_basis_id in per_assignments_f, employee's effective stat date and end date
   -- must be in range of pay_basis_id's start date and end date.
   -- If the above is not true, you must skip to call api, otherwise api will throw you out.
   -- currently paY_basis_id 84, it start date is 01-JAN-04, so all emp have effective start date before
   -- 01-JAN-04 will have problem - the following code is just for bandage solution..
*/

--  ---------------------------- bandage solution --------------------------
/*
IF (trunc(xSal.hire_date) < trunc(to_date('01-JAN-04'))) Then
   l_sal_err_flag := TRUE;
...
--  ---------------------------- bandage solution --------------------------
               ...
               IF (NOT l_error  AND NOT l_sal_err_flag) THEN
			       ...
			       p_validate                               => l_validate,
                               ...
                               p_change_date                           	=> xSal.effective_start_date,
				 ...
				 --UPDATE cust.ttec_h_mx_sal				-- Commented code by IXPRAVEEN-ARGANO,19-May-2023
				 UPDATE apps.ttec_h_mx_sal                  --  code Added by IXPRAVEEN-ARGANO,   19-May-2023
                                 SET
				...
				EXCEPTION
				      ...
				      WHEN OTHERS THEN
				       ...
				       l_sal_err_flag := TRUE ;
                                       ...
                                       g_error_message := SQLERRM;
                                       l_error_message :=  'ERROR while update ttec_h_mx_sal temporary talbe ';
                                               apps.ttec_conv_util.Stampln(e_open_log_file, MODULE_NAME_CONST||' '|| l_error_message
                                       || xSal.employee_number_in_ls ||' - '|| g_error_message||' '||SYSTEM_USER_CONST);
