create or replace PROCEDURE      ttec_gl_tcmb_upload_rates (errbuf OUT VARCHAR2, retcode OUT VARCHAR2)
IS
/******************************************************************************
   NAME:       TTEC_GL_TCMB_UPLOAD_RATES
   PURPOSE:    Loads the Turkish TCMB Rates daily rates into the Staging
               table and further into GL.GL_DAILY_RATES_INTERFACE table.
   CONCURRENT PRG: TeleTech TCMB Exchange Rates - Upload Rates to Oracle
   REVISIONS:
   Ver        Date        Author                Description
   ---------  ----------  ---------------       ------------------------------------
   1.0        7/11/2011   Kaushik Gonuguntla    Initial Script
   ...
   IS
      SELECT base_from_currency, quote_to_currency, amount, conversion_date, ask / amount ask,
        ...
        FROM (SELECT SUBSTR (file_string, 1, 3) base_from_currency,
                     ...
                     file_process_time
                FROM ttec_tcmb_daily_rates_parsing
...
BEGIN
   BEGIN
      --DELETE FROM cust.ttec_daily_exhg_rates_stg;  --code commented by RXNETHI-ARGANO,17/07/23
      DELETE FROM apps.ttec_daily_exhg_rates_stg;    --code added by RXNETHI-ARGANO,17/07/23
   ...
   BEGIN
      SELECT COUNT (*)
        INTO v_oanda_cnt
        --FROM cust.ttec_oanda_daily_rates_ext_stg   --code commented by RXNETHI-ARGANO,17/07/23
        FROM apps.ttec_oanda_daily_rates_ext_stg     --code added by RXNETHI-ARGANO,17/07/23
       WHERE file_process_time > TO_CHAR (SYSDATE - .25, 'YYYYMMDDhhmm');
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
         v_oanda_cnt := 0;
   ...
   BEGIN
      SELECT COUNT (*)
        INTO v_tcmb_cnt
        --FROM cust.ttec_tcmb_daily_rates_parsing;   --code commented by RXNETHI-ARGANO,17/07/23
        FROM apps.ttec_tcmb_daily_rates_parsing;     --code added by RXNETHI-ARGANO,17/07/23
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
         v_tcmb_cnt := 0;
   ...
   THEN
      BEGIN
         ...
         --INSERT INTO cust.ttec_daily_exhg_rates_stg   --code commented by RXNETHI-ARGANO,17/07/23
         INSERT INTO apps.ttec_daily_exhg_rates_stg     --code added by RXNETHI-ARGANO,17/07/23
      ...
      EXCEPTION
         WHEN OTHERS
         THEN
            fnd_file.put_line (fnd_file.LOG, 'Inserting Oanda Rates to Staging Table Failed');
      END;
      ...
      THEN
         ...
         LOOP
            BEGIN
               ...
               SELECT 'Y'
                 INTO v_exist
                 --FROM cust.ttec_daily_exhg_rates_stg   --code commented by RXNETHI-ARGANO,17/07/23
                 FROM apps.ttec_daily_exhg_rates_stg     --code added by RXNETHI-ARGANO,17/07/23
               ...
               THEN
                  ...
                  --UPDATE cust.ttec_daily_exhg_rates_stg  --code commented by RXNETHI-ARGANO,17/07/23
                  UPDATE apps.ttec_daily_exhg_rates_stg    --code added by RXNETHI-ARGANO,17/07/23
            ...
            EXCEPTION
               ...
               THEN
                  fnd_file.put_line (fnd_file.LOG,
                                        'Update TCMB Not Successful (No Matching Currency available):'
                                     || r_tcmb_curr_upd.base_from_currency
                                     || '-'
                                     || r_tcmb_curr_upd.quote_to_currency
                                     || '-'
