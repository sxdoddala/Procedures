create or replace PROCEDURE TTEC_H_BR_CNV_ADD AS
  -- -----------------------------------------------------------------------------
  -- -----------------------------------------------------------------------------
  -- Program Name  : TTEC_H_BR_CNV_ADD
...
e_log_file_name                 VARCHAR2(20)   := 'HR_ADD_BR_log_file_';
...
FROM  cust.ttec_h_br_add a,
	cust.ttec_h_br_emp b
*/
--code added by RXNETHI-ARGANO
FROM  apps.ttec_h_br_add a,
	apps.ttec_h_br_emp b
...
/*
		l_error_message := 'Create Address Process - Brazil';
...
--  ---------------------------------------------------------------------
BEGIN
	IF ltrim(rtrim(addrec.validate_county))='BR'THEN
...
IF (v_address_id is not null) THEN
			--UPDATE cust.ttec_h_br_add    --code commented by RXNETHI-ARGANO,19/05/23
			UPDATE apps.ttec_h_br_add      --code added by RXNETHI-ARGANO,19/05/23
				SET
